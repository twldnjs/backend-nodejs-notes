...
SELECT *, (SELECT * FROM likes WHERE books.id=liked_book_id) AS likes FROM books;
...
// 개별 도서 조회 시, 사용자가 좋아요 했는지 여부 포함된
SELECT * FROM books WHERE books.id = 1;
SELECT * FROM likes WHERE user_id=1 AND liked_book_id=1;
